...
- RecordEpisodeStatistics wraps env to provide info["episode"] stats.
...
For each episode:
1. env.reset(seed=config["env"]["seed"] + episode)
2. Step loop:
...
3. Log data:
   ...
   - steps from info["episode"]["l"]
   - termination_cause

## Outputs

- Videos: outputs/videos (one per episode)
- CSV log: outputs/logs/<name>_eval_log.csv

## Notes

- DQN policy loads weights from outputs/models/<name>_model.pth.
- Termination cause is derived from the last reward and final state.
