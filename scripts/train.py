...
# Network
from src.policies.DQN_policy import DQN_network
...
pd.DataFrame(data).to_csv("outputs/logs/" + config["name"] + "_log.csv", index=False)
torch.save(policy_net.state_dict(), "outputs/models/" + config["name"] + "_model.pth")
